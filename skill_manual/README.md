...
| `library-package` | npm / PyPI / crates / go module として公開するライブラリ | ✅ |
| `production-saas` | Next.js / Supabase 等、認証・決済を伴う商用アプリ | ✅ |
| `mobile-app` | iOS / Android / React Native / Flutter のモバイルアプリ | ✅ |
| `ml-data` | Jupyter + pytest + DVC/MLflow 等の ML パイプライン | ✅ |
| `infra-iac` | Terraform / Ansible / K8s manifests / Helm chart | ✅ |
...
cd ~/harness-forge && make install                   # ~/.claude/skills/ に symlink
```

`make` が無い環境では `bash install.sh` でも同じ動作。Makefile target 一覧:
...
**フェーズ**: Phase 9 完了 / 2026-04-26 時点 — 全 6 archetype + packaging
**実装済**: daily-utility / library-package / mobile-app / production-saas / infra-iac / ml-data の end-to-end scaffold + Validator 整合性チェック + `make install` / `.claude-plugin/plugin.json` (marketplace 配布対応)
**残タスク (将来)**: runtime knowledge refresh (`--refresh-knowledge`), `harness-evolver` (既存 harness 進化), 多 locale 対応
